...
const sections: CaseStudySection[] = [
	{
		...
		paragraphs: [
			...
			"So I started building it. After a few weeks of using it myself, it became obvious that this was not only my problem, and Glotsmith became a product rather than a tool.",
			"That decision is where the interesting part starts, because building it for myself turned out to be the easy half.",
		],
		image: {
	...
	},
	{
		...
		paragraphs: [
			"Building the thing I wanted took roughly two months. Vue 3 and Vite on the front end, Node and Express behind it, PostgreSQL underneath, OCR, translation, and speech from Google Cloud. Workbooks made of sections, translation rows, vocabulary attached to a course, rich text notes, PDFs rendered in the browser. It worked, and I used it every week.",
			"Making it into something a stranger could pay for took another four months, and almost none of that was product work.",
			"This is the part nobody warns you about, and it is the honest answer to what was hardest. It was not a technical problem. It was deciding to keep going, over and over, once it became clear how much of the work had nothing to do with the thing I had set out to build.",
			"I was figuring it out as I went, and every week produced another obligation I had not known existed.",
	...
	},
	{
		...
		paragraphs: [
			...
			"So I restructured as a self-employed sole trader, which was a far more proportionate footing for a product with no users yet.",
			"That single decision invalidated the payment integration I had already built and tested. As a freelancer, I could not handle invoicing the way the Stripe setup assumed, and the accountancy costs the restructure was meant to avoid would have come straight back through the payment model instead.",
			"Stripe was not the wrong tool because of anything about Stripe. It was the wrong tool because of a decision made two layers away from the code.",
			...
			"That migration took a long time, and I did it after I had already finished the Stripe work.",
			"I do not regret the outcome. I do regret the order.",
	...
	},
	{
		...
		paragraphs: [
			"Working alone means the scarcest resource is your own time, so most of the real decisions are about what to defer.",
			"I costed a full TypeScript migration properly, file by file, across roughly 41,000 lines of production code. It came out at somewhere between fifty and sixty working days. It genuinely improves the codebase, and it is planned for an upcoming release. What it was not was something to do before launch, when those two months would have come directly out of shipping.",
			"The value of costing it was knowing that number rather than guessing at it.",
			"The infrastructure decisions went the same way. What I ended up running is deliberately boring: a single EC2 instance running Docker Compose behind Cloudflare, with a managed PostgreSQL instance and S3.",
			"No Kubernetes and no Redis, because at this size neither justifies its operational cost. Both are on the path for scaling out later, and the application is already shaped for it. Sessions live in the database rather than in memory, and scheduled jobs elect a leader through a database advisory lock, so a second instance can be added without a rewrite when the load justifies one.",
			"The compliance work was the part that surprised me most. A published privacy policy is not just a document. It is a set of factual claims about a running system, and keeping those claims true after launch turned out to be real engineering rather than paperwork.",
			"Data retention, consent records, copyright takedown, and the obligations that come with selling into both the EU and the US all ended up as code rather than text on a page.",
		],
	},
	{
		id: "what-i-got-wrong",
		...
		paragraphs: [
			"I built the entire product without speaking to a single potential customer.",
			"No validation, no market research, no conversations. I needed this thing, I enjoyed building it, and I assumed that was enough.",
			"It might be. It might not.",
			"I did not find out until registrations opened, having spent six months learning something a few dozen conversations could have told me in a fortnight.",
			"Just because I need something does not mean anyone else does.",
			"Next time, the conversations come first.",
		],
	},
	{
		id: "where-it-is-now",
		...
		paragraphs: [
			"Everything is built, tested, and deployed. It runs on AWS with staging and production environments, deployment through GitHub Actions, automated database snapshots before every migration, and a scripted rollback.",
			"Roughly 41,000 lines of production code sit behind a 20,000-line test suite with an 85% coverage gate in CI.",
			"The final legal review is complete, and registrations opened on 18 August 2026.",
			"If you are thinking about doing something similar, the only advice I have is to keep going. You will hit a lot of moments where it seems pointless, and the difficulty is rarely the code.",
			"But even if the product fails, the six months are not wasted, because everything in this piece is something I did not know how to do a year ago.",
